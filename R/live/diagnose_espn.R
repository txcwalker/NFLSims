...
.in_window_now <- function(tz = "America/Chicago",
  ...
  if (!any(day_mask, na.rm = TRUE)) {
    return(list(in_window = FALSE, reason = sprintf("[LiveToday] No games today (%s).", today)))
  }
  ...
  # FIXED: Use sprintf instead of glue to avoid encoding issues
  reason_msg <- sprintf(
    "[LiveToday] Window %s-%s local; now=%s",
    format(start_at, "%H:%M"),
    format(end_at, "%H:%M"),
    format(now, "%H:%M")
  )
...
run_live_today <- function(tz = "America/Chicago",
  ...
  # NEW: Add diagnostic for ESPN data availability
  if (Sys.getenv("DEBUG_ESPN", "0") == "1") {
    cat("\n[DEBUG] Testing ESPN data availability...\n")
    source("R/live/espn_adapter.R", local = TRUE)

    test_ids <- espn_active_event_ids(only_live = FALSE)
    cat("[DEBUG] Found", length(test_ids), "event IDs\n")

    if (length(test_ids) > 0) {
      cat("[DEBUG] Testing PBP for first event:", test_ids[1], "\n")
      test_pbp <- espn_fetch_pbp(test_ids[1])
      cat("[DEBUG] PBP returned", nrow(test_pbp$plays), "plays\n")

      if (nrow(test_pbp$plays) == 0) {
        cat("[DEBUG] WARNING: Event exists but has 0 plays!\n")
        cat("[DEBUG] This usually means:\n")
        cat("[DEBUG]   - Game is in pregame/warmups (not started yet)\n")
        cat("[DEBUG]   - ESPN PBP endpoint structure changed\n")
        cat("[DEBUG]   - Game just finished and plays were cleared\n")
      }
    }
    cat("\n")
  }
