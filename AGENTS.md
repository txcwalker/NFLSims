...
<!-- Audit H2 (2026-09-22): this section used to be ~68 lines of full session
narrative, every bullet ending "Full detail: WORKLOG.md <date> entry" -- i.e.
100% duplicated there. Trimmed to a pointer + the genuinely-still-open
constraints, pulled forward as of the 2026-09-04 session (the last time this
section was updated) and NOT re-verified against current calibration --
confirm before trusting any specific number below as still true. -->

**Full session-by-session history lives in [WORKLOG.md](WORKLOG.md)** (reverse-chronological, one entry per session). This section is deliberately short now — read WORKLOG for the detail behind any item below.

**A 2026-09 full-repo audit ran just before Week 1** — see [docs/audit/2026_09_audit/](docs/audit/2026_09_audit/) (phase_0 through phase_7 + `fix_pass_plan.md`, the batched remediation plan). Safety/tooling/docs batches (A, C, H) are being worked through starting 2026-09-22; dead-code removal (B), the engine test suite + validation benchmark (D, F), and publish-prep (G) are deliberately deferred until later in the season — do not assume any Batch B/D/F/G item is done without checking `fix_pass_plan.md` directly.

**Known open threads, not yet re-verified as of this trim:**
- Completion % was running ~68% vs. the calibrated 64.40% target as of the 2026-07-22 session (same root-cause family as the sack drift below — Phase 1's DNA rebuild touching `catch_rate`/`adot`/`contested_catch_rate`/etc.), not yet investigated. The audit's E1 item (dropback-label retrain, done 2026-09-09) targets pass *volume*, not completion % directly — check current state before assuming this is fixed.
- Sack rate was recalibrated to ~2.39-2.41/game (target 2.41) as of 2026-07-22 via `SACK_PROB_CALIBRATION_MULT = 0.494`, but an earlier 2026-07-21 note flagged sacks running "~5-15% hot" pre-recalibration as a possible future Gate 2 retrain candidate — status unclear, re-check before trusting either number.
- Air yards gap (−4.4% to −7.3% by zone/depth) — open as of Round 11, same orphaned-training-script fix pattern that fixed YAC (Round 9) would likely apply if tackled.
- PAT/2-point-conversion modeling — every TD still awards a flat +7 unconditionally, no PAT/2pt logic anywhere in the engine (`docs/models/README.md` §2.14). Was flagged "must land before season launch" — season has now started with this still not built; needs a decision on priority, not a silent fix.
- Throwaway mechanism rework — deferred, parameters already agreed (target ~5% of dropbacks, 75/25 pressure/clean-pocket split). See §11.6.
- Injury stoppages / other administrative clock stoppages (replay review, etc.) — deferred to v0.4.0, not started, do not start without Cam's go-ahead.
- `data/overrides/2026/` (the real roster hand-tune source of truth) was untracked as of 2026-09-04 and flagged as "should probably get committed" — check `git status` before assuming it's safe.
- Coach-aggression-scales-off-`deep_shot_rate` design (`self.coach_aggression`) — Cam's not fully sold on it, still live, deferred to off-season revisit.

**Resolved, kept for context:** OT reachability, kneel-down on 4th down, fumble-vs-TD ordering, punt-return re-clipping, YAC calibration (Round 9), the v0.4.0 trench-tier rebuild (run + pass, both sides), the Q2→Q3 quarter-transition state-corruption bug (Round 13), and the Python 3.8→3.12 migration are all done — see WORKLOG for any of these if you need the mechanism, not just the fact.
...
scripts/
  ...
  roster_management/                            ← 2026 Roster & DNA Blending Pipeline (2026-07-22), see §0. Build order matters -- see WORKLOG.md's 2026-07-22 entry for the full phase sequence.
    ...
    dfs_status_ledger.py                          ← 2026-09-22: persistent, STICKY Game Explorer Active/Inactive toggles (data/overrides/2026/dfs_status_ledger.json). Latest entry at/before a week wins, so a toggle carries into later weeks; "active" on an IR/PUP player overrides that stint (dfs_status=force_active), never a cut/left_team player. Read by build_week_overrides_v_0_1_0.build_team_week_rows (ledger=) and written by POST /api/dfs/roster_status. `python dfs_status_ledger.py seed <week>` imports a pre-ledger week's sheet "out"s (opt-in, not yet run).
...
```bash
# Full test suite -- 121 tests as of 2026-09-22 (bare `pytest` now works too,
# scoped to tests/ via pyproject.toml -- see audit fix-pass C1)
...
- **6 coach-tendency levers are computed, stored, hand-editable, and read by nothing (audit S3-9, 2026-09).** `no_huddle_rate`, `sec_per_play`, `air_yards_tendency`, `screen_rate`, `play_action_rate`, `rpo_rate`, `conservative_score_bias` live in `data/dna/coach_coordinator_levers_2026.csv` / `coach_dna.json` / `{TEAM}_traits_2026.json` but are consumed by nothing in `src/nfl_sim/` or `src/api/` — hand-tuning any of them has zero sim effect. Only `proe` and `deep_shot_rate` are live. Cam's call: keep them, may wire in later — don't "fix" by deleting. Full detail: [data/dna/coach_coordinator_levers_2026_notes.md](data/dna/coach_coordinator_levers_2026_notes.md).
